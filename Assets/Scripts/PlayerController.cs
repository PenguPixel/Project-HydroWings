...
public class PlayerController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // Verhindert, dass der Player weiter gegen die obere Grenze gedrückt wird.
        if (currentPosition.y >= MaxCharacterOffsetY && targetVelocity.y > 0f)
        {
            targetVelocity.y = 0f;
        }

        // Verhindert, dass der Player weiter gegen die untere Grenze gedrückt wird.
        if (currentPosition.y <= MinCharacterOffsetY && targetVelocity.y < 0f)
        {
            targetVelocity.y = 0f;
        }
        ...
        float smoothedVelocityY = Mathf.MoveTowards(
            currentVelocity.y,
            targetVelocity.y,
            acceleration * Time.fixedDeltaTime
        );
        ...
        currentPosition.x += _camSpeed * Time.fixedDeltaTime;
        currentPosition.y = Mathf.Clamp(currentPosition.y, MinCharacterOffsetY, MaxCharacterOffsetY);
